...
public class DtoCoberturaBaseTest {
  ...
  protected void generalTest(List<Class<?>> dtoClasses) {
    ...
    for (Class<?> dtoClass : dtoClasses) {
      ...
      Map<String, ClassUtil.FieldBean> fieldsMap = ClassUtil.getFieldsMap(dto.getClass());
      ...
      for (int i = 0; i < dtoClones.length; i++) {
        for (int j = 0; j < dtoClonesInv.length; j++) {
          dtoClones[i].equals(dtoClonesInv[j]);
          dtoClonesInv[j].equals(dtoClones[i]);
        }
      }
      dto.equals(other);
    ...
    if (String.class.isAssignableFrom(clz)) {
      ...
      if (cs.length == 0) {
        ...
        for (int i = 0; i < cs.length; i++) {
          ...
          Parameter[] paramDefs = cs[i].getParameters();
          ...
          try {
            ...
            throw new RuntimeException("Error al crear instancia. Constructor: " + cs[i], e);
          }
  ...
  private void clone(Object fromObj, Object toObj, Map<String, ClassUtil.FieldBean> fieldsMap,
      int idx) {
